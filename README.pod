...
or even to other bare-metal embedded architectures.

The makefile that builds the B<GCC-based toolchain> is completely generic (it knows nothing about the Arduino Due),
so that you can use it to create toolchains for other ARM Cortex microcontrollers, like the STM32 family.
...
You will find more information about the Arduino Due on my wiki page at S<< L<< http://rdiez.shoutwiki.com/wiki/Hacking_with_the_Arduino_Due >> . >>
...
=item * L<< C++14|https://en.wikipedia.org/wiki/C%2B%2B14 >> support, including the L<< STL library|https://en.wikipedia.org/wiki/Standard_Template_Library >>.
...
See the toolchain builder makefile for more information.

=item * L<< Newlib|https://sourceware.org/newlib/ >> with malloc support.
...
=head2 About the Toolchain Builder Makefile
...
L<< https://developer.arm.com/open-source/gnu-toolchain/gnu-rm >>
...
You will find more information about my experience with the Arduino Due on my wiki page at S<< L<< http://rdiez.shoutwiki.com/wiki/Hacking_with_the_Arduino_Due >> . >>
...
Pre-built binary files are available in a separate repository at S<< L<< https://github.com/rdiez/JtagDueBinaries >> . >>
...
Change to the 'Toolchain' subdirectory and type "make help" for detailed information.

Install the prerequisites first. On Ubuntu/Debian systems, installing package 'build-essential' gets you a long way.

On Windows/Cygwin, install these packages:

  gcc-core gcc-g++ automake make curl patch libtool
...
For a toolchain with GCC 7.3, the bin directory needs 169 MB, and the build directory 2.1 GB. But sizes can vary depending on the build flags.

=item 2) Download and unpack the Atmel Software Framework into some directory of your choice.

I have tested against ASF versions 3.19.0.95, 3.35.1.54 and 3.40.0. You will probably have to manually edit file
...
Copyright (C) R. Diez 2012-2018,  rdiezmail-arduino at yahoo.de
