...
Alternatively, create a Linux I<< udev >> rule in order to get the same "/dev/jtagdue1" name every time.
...
You will need to disconnect the command console from the USB virtual serial port beforehand, if you have it open.
...
If you are connecting to a second Arduino Due target, check out the files under F<< OpenOCD/SecondArduinoDueAsTarget/ >> .
