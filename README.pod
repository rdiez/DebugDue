...
=item * L<< C++11|https://en.wikipedia.org/wiki/C%2B%2B11 >>.
...
you should configure the usual compile/run/etc. keys to run the script with different arguments. For example:
 ...
 F5 (run)    :          ./JtagDueBuilder.sh --build --program-over-jtag --debug --debugger-type=ddd
 Ctrl+Alt+F7 (rebuild): ./JtagDueBuilder.sh --clean --build
 Ctrl+R (run to line):  ./JtagDueBuilder.sh --build --program-over-jtag --debug --add-breakpoint="Main.cpp:123"
