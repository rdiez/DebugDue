...
=item * L<< Modern C++17|https://en.wikipedia.org/wiki/C%2B%2B17 >> and higher support, including the L<< STL library|https://en.wikipedia.org/wiki/Standard_Template_Library >>.
...
=item * Many automatic checks to make sure the compiler its libraries are correctly configured for embedded development.
...
=item * The build script can tell which C and C++ modules were needlessly compiled, because they were completely discarded during linking.

This information helps optimise compilation times in big projects.
