...
=item * In order to help trim the firmware size, the build script can optionally generate a disassembly, an object list sorted by size and a list of all strings found in the final binary.

=item * If L<< ccache|https://en.wikipedia.org/wiki/Ccache >> is detected, it will be used in order to reduce compilation times.
