...
programming environment for the Arduino Due, with the following features:
...
Useful generic commands have been implemented: memory usage, CPU load in the last 60 seconds, compiler version used to build, last reset cause, uptime.

=item * Library of common functions.

Most basic functions you need to get started have been implemted: Busy wait loop helpers, basic text parsing, I/O utilities, stack usage tools, uptime calculation, circular buffer, etc.

=item * Start-up delay for easy JTAG debugging right after reset.

=item * Firmware cross-compiled with the standard autotools.
...
compared to 8 KiB/s with the Bus Pirate. Note that I had to manually patch OpenOCD 0.8.0 as of May 2013 in order
...
Usage example:
  ...
  bossac --port=ttyACM0 --force_usb_port=false --write "filename.bin" --boot=1 --reset
