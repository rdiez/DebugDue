...
B<bare-metal C++ programming environment> for the Arduino Due and, in the future, for similar boards.

The script that builds the B<GCC-based toolchain> is completely generic (it knows nothing about the Arduino Due),
so that you can use it to create toolchains for other ARM Cortex microcontrollers, like the STM32 family.
In fact, the toolchain builder script is probably the most valuable outcome of this project.
...
You will find more information about the Arduino Due on my wiki page at L<< http://rdiez.shoutwiki.com/wiki/Hacking_with_the_Arduino_Due >>S< .>
...
F<< sam/utils/compiler.h >> and surround the definitions of macros I<< min >> and I<< max >> with I<< #ifndef __cplusplus >>S< ,>
...
This kind of fix is in place for F<< sam0/utils/compiler.h >>S< ,> but not for F<< sam/utils/compiler.h >>S< ,> at least for ASF S<version 3.34.2.53>.
...
Copyright (C) R. Diez 2012-2017,  rdiezmail-arduino at yahoo.de
