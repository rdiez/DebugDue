...
Restarting the firmware under the debugger is almost instantaneous.
...
Edit the generated 'configure' script that Autoconf generates, and remove the I<< --with-ext >> and I<< --without-ext >> arguments from this line:
...
Build OpenOCD. For example:
  ...
  ./configure --enable-buspirate --prefix="$HOME/SomeDir/openocd-0.10.0-bin"
